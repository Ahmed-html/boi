...
exports.run = (client, message, args) => {
	const notice3 = new Discord.MessageEmbed()
		.setDescription(
			`:x: Failed to set slowmode in this channel, check your slowmode number!`
		)
		.setColor("RED");
	...
	const noticwsse1 = new Discord.MessageEmbed()
		.setDescription(
			`:x: You can only type in 0 - 21600 seconds!`
		)
		.setColor("RED");

	const notice22 = new Discord.MessageEmbed()
		.setDescription(
			`:x: I don't have permission to change channel slowmode!`
		)
		.setColor("RED");
